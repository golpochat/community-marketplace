...
export async function runDevUsersSeed(prisma: PrismaClient): Promise<DevUsersSeedResult> {
  ...
  for (const entry of DEV_BOOTSTRAP_USERS) {
    ...
    const existingById = await prisma.user.findUnique({ where: { id: entry.id } });
    if (existingById && existingById.email !== entry.email) {
      console.log(
        '[dev-users-seed] Skipping %s — id already used by %s',
        entry.email,
        existingById.email,
      );
      continue;
    ...
    const existing =
      (await prisma.user.findUnique({ where: { email: entry.email } })) ?? existingById;
