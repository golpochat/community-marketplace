  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div
        role="dialog"
        aria-modal
        className="flex max-h-[80vh] w-full max-w-lg flex-col rounded-xl bg-white shadow-lg"
      >
        <div className="border-b border-gray-200 px-6 py-4">
          <h2 className="text-lg font-semibold text-gray-900">Status history</h2>
          {listingTitle && <p className="mt-1 text-sm text-gray-600">{listingTitle}</p>}
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {loading && <p className="text-sm text-gray-600">Loading history…</p>}
          ...
          <ul className="space-y-4">
            {entries.map((entry) => (
              <li key={entry.id} className="rounded-lg border border-gray-100 px-4 py-3 text-sm">
                ...
                {entry.reason && (
                  <p className="mt-1 text-xs text-gray-700 whitespace-pre-wrap">{entry.reason}</p>
                )}
              </li>
            ))}
          </ul>
