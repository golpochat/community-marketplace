...
import { DisputeEvidenceUpload } from '@/components/disputes/dispute-evidence-upload';
...
export function BuyerDisputeDetailPage({ disputeId }: { disputeId: string }) {
  ...
  const canUpload = ACTIVE_STATUSES.has(dispute.disputeStatus);
    ...
    >
      ...
      <div className="grid gap-6 lg:grid-cols-3">
          ...
          <Card title="Details">
            <dl className="grid gap-3 text-sm sm:grid-cols-2">
              ...
              {dispute.resolutionNotes ? (
                <div className="sm:col-span-2">
                  <dt className="text-[hsl(var(--dashboard-sidebar-muted))]">Resolution</dt>
                  <dd className="mt-1 whitespace-pre-wrap text-[hsl(var(--dashboard-main-fg))]">{dispute.resolutionNotes}</dd>
                </div>
              ) : null}
            </dl>
          </Card>
        ...
        <div className="space-y-6">
          {canUpload ? (
            <DisputeEvidenceUpload disputeId={dispute.id} onUploaded={() => void load()} />
          ) : null}
