...
type StepId = 'seller_type' | 'verification' | 'payouts' | 'first_listing';
...
export function SellerSetupWorkflow() {
  ...
  const steps = useMemo((): WorkflowStep[] => {
    ...
    const raw: Omit<WorkflowStep, 'state'>[] = [
      ...
      },
      {
        ...
        description: identityPending
          ...
          : 'Required before you can receive payments',
      },
      {
        id: 'payouts',
    ...
    const completion = [sellerTypeComplete, verificationComplete, payoutsComplete, listingComplete];
    const firstIncomplete = completion.findIndex((done) => !done);

    return raw.map((step, index) => {
      let state: StepState = 'upcoming';
      if (completion[index]) state = 'complete';
      else if (firstIncomplete === index) state = 'current';
      return { ...step, state };
    });
  }, [started, identityComplete, identityPending, payoutsReady, hasListings]);
  ...
  const setupLoading = verificationLoading || connectLoading || listingsLoading;
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {phase === 'buyer_only'
                ? 'One guided path — choose how you sell, verify, connect payouts, then list.'
        ...
        {!setupLoading || currentStepId === 'seller_type' ? (
          <>
              ...
              <div className="space-y-4">
                <p className="text-sm text-muted-foreground">
                  {identityPending
                    ...
                    : 'Confirm your identity so buyers can trust who they are buying from. This usually takes a few minutes.'}
                </p>
            ...
            {currentStepId === 'first_listing' && (
              <div className="space-y-4">
                <p className="text-sm text-muted-foreground">
                  You are ready to sell. Create your first listing — you can save a draft and publish
...
export function SellerSellingPageHeader() {
  ...
  return (
    <PageHeader
      title="Start selling"
      description="One account for buying and selling. Complete each step below — we will guide you through."
    />
  );
}
