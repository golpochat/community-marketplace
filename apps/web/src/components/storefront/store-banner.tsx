...
function StorefrontBadge() {
  return (
    <div className="absolute bottom-4 right-4 z-10 flex items-center gap-1.5 rounded-full border border-border/80 bg-white/90 px-3 py-1.5 text-xs font-medium text-muted-foreground shadow-sm backdrop-blur-sm">
...
export function StoreBanner({ bannerUrl, name }: StoreBannerProps) {
  if (bannerUrl) {
    return (
      <div className={`relative overflow-hidden bg-muted ${STOREFRONT_HERO_BANNER_CLASS}`}>
  ...
  return (
    <div
      className={`relative overflow-hidden bg-gradient-to-br from-muted via-background to-primary/5 ${STOREFRONT_HERO_BANNER_CLASS}`}
      role="img"
