    ...
    >
      ...
      {PILOT_FEEDBACK_FORM_URL ? (
        <>
          ...
          <p className="mt-4">
            For urgent safety issues outside a listing, use <strong>Report user</strong> on the
            relevant profile or email us directly.
          </p>
        </>
      ) : (
        <p className="mt-8">
          For bugs or account issues, email{' '}
          <a href={`mailto:${PLATFORM_SUPPORT_EMAIL}`} className="text-primary hover:underline">
            {PLATFORM_SUPPORT_EMAIL}
          </a>
          . Safety on a listing: use <strong>Report listing</strong> on that page.
        </p>
      )}
