...
#   .\scripts\smoke-pilot.ps1 -BaseUrl "http://localhost:4000" -LoginEmail "seller@community.market" -LoginPassword $pwd
...
if ($LoginEmail -and $LoginPassword) {
    ...
    $plainPassword = Get-PlainTextFromSecureString -Value $LoginPassword

    try {

        $body = @{ email = $LoginEmail; password = $plainPassword } | ConvertTo-Json
